...
- **Styles can target V4.5 and/or V5.** Style Editor → *Works with* chips (both by default; each list row shows a badge). The STYLES panel, cascade beats and keyboard style cycling only list styles made for the active model, with an "N styles hidden (made for V5)" hint so nothing looks lost; switching models swaps an active style made only for the other family for that family's default style and never touches one that targets both. Older style files, packs and imports load as "both"; the bundled styles are marked.
- **Enhance "Max" on V5.** A MAX ✨ chip after the scale chips (offered when the source is below 0.8 × NovelAI's 3,145,728 px cap) sends the source at its own size with `upscaled_enhance: true` and gets it back scaled to ~3.1 MP; the dimension line shows the predicted size. The numeric scale chips now follow NovelAI's 2× / 1.5× / 1× rule instead of a fixed pair. The Anlas figure next to the dimensions is *estimated at the output pixel count* — NovelAI has not published what Max costs — and the request format has not yet been exercised against the live API.
